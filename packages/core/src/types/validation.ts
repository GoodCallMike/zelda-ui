...
export const validateStatus = (
  status: unknown,
): status is 'error' | 'warning' | 'success' | 'info' =>
  isString(status) && ['error', 'warning', 'success', 'info'].includes(status);

export const validateVariant = (
  variant: unknown,
): variant is 'primary' | 'secondary' | 'default' | 'ghost' | 'outline' =>
  isString(variant) &&
  ['primary', 'secondary', 'default', 'ghost', 'outline'].includes(variant);

// Theme validation
export const validateColorScheme = (
  scheme: unknown,
): scheme is 'light' | 'dark' | 'auto' =>
  isString(scheme) && ['light', 'dark', 'auto'].includes(scheme);

export const validateBreakpoint = (
  breakpoint: unknown,
): breakpoint is 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl' =>
  isString(breakpoint) &&
...
export const createEnumValidator =
  ...
  (value: unknown): value is T =>
    isString(value) && validValues.includes(value as T);

export const createArrayValidator =
  <T>(itemValidator: (item: unknown) => item is T) =>
  (value: unknown): value is T[] =>
    isArray(value) && value.every(itemValidator);
...
export const assertIsFunction = (
  ...
  if (!isFunction(value)) {
    throw new TypeError(
      `Expected ${name} to be a function, got ${typeof value}`,
    );
  }
};
